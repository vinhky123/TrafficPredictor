...
export function Sidebar({ segments, active, onSelect }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return segments.filter(
      (seg) =>
        seg.name.toLowerCase().includes(s) ||
        String(seg.segment_index).includes(s),
    );
  }, [segments, q]);

  async function runPredict(seg: RoadSegment) {
    setLoading(true);
    setData(null);
    try {
      const resp = await postPredict(seg.segment_index);
      setData(resp);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <aside className="flex h-full flex-col gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur">
      <div className="space-y-1">
        <div className="text-sm font-semibold">Road Segments</div>
        <div className="text-xs text-zinc-400">
          Pick a road segment to see current speed + forecast.
        </div>
      </div>
            ...
            >
              <div className="min-w-0">
                <div className="truncate text-sm font-medium">
                  {seg.name || `Segment #${seg.segment_index}`}
                </div>
