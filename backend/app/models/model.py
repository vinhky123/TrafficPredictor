...
_DEFAULT_MODEL_STATE_PATH = os.path.join(os.path.dirname(__file__), "..", "notebook", "TimeXer.pth")
num_valid_columns = 8
...
class GetModel(object):
    def __init__(self, model_state_path: str | None = None):
        model_state_path = model_state_path or _DEFAULT_MODEL_STATE_PATH
        self.model = TimeXer(
            seq_len=96,
            patch_len=12,
            patch_num=8,
        ...
        # `weights_only` is available in newer torch; keep backward compatibility.
        try:
            state_dict = torch.load(
                model_state_path,
                map_location=torch.device("cpu"),
                weights_only=True,  # type: ignore[call-arg]
            )
        except TypeError:
