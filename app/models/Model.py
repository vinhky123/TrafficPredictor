...
class Attention(nn.Module):
    def __init__(self, d_model, n_head=4, dropout=0.1):
        ...
        self.multihead_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=n_head, dropout=dropout)
...
class TX_EncoderLayer(nn.Module):
    ...
    def forward(self, x, cross):
        ...
        cross_attn = torch.reshape(cross_attn, (cross_attn.shape[0] * cross_attn.shape[1], cross_attn.shape[2])).unsqueeze(1)
...
class TX_Encoder(nn.Module):
    def __init__(self, d_model, num_layers):
        ...
        self.e_layers = nn.ModuleList([TX_EncoderLayer(d_model) for _ in range(num_layers)])
...
class TimeXer(nn.Module):
    def __init__(self, seq_len, patch_len, patch_num, num_variate, pred_len, use_norm=True, d_model=256, num_layers=6):
        ...
        self.en_embedding = EnEmbedding(self.d_model, self.patch_len, num_variate=self.num_variate)
    ...
    def forward(self, x):
        ...
        encoder_out = torch.reshape(encoder_out, (-1, n_vars, encoder_out.shape[-2], encoder_out.shape[-1]))
